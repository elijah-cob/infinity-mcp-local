...
let RUNTIME_API_KEY: string | undefined = readArg("apiKey") || ENV_API_KEY;
...
function normalizeBaseUrl(): string | undefined {
  if (!ENV_BASE_URL) return undefined;
  // Ensure base ends with /api/v2
  const base = ENV_BASE_URL.replace(/\/$/, "");
  if (base.endsWith("/api/v2")) return base;
  if (base.endsWith("/api")) return `${base}/v2`;
  return `${base}/api/v2`;
}

function createInfinityClient(apiKeyOverride?: string): AxiosInstance {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    switch (name) {
      ...
      case "infinity_ping": {
        const client = createInfinityClient(args?.apiKey as string | undefined);
      ...
      case "infinity_get_item": {
        ...
        const expand = Array.isArray(args?.expand) ? (args?.expand as string[]) : undefined;
        if (expand && expand.length > 0) {
          expand.forEach((v, i) => { (query as any)[`expand[${i}]`] = v; });
        }
      ...
      case "infinity_get_items_bulk": {
        ...
        async function worker(id: string): Promise<{ id: string; data?: any; error?: any }> {
          try {
            const query: Record<string, any> = {};
            if (expand && expand.length > 0) {
              expand.forEach((v, i) => { (query as any)[`expand[${i}]`] = v; });
        ...
        const results: { id: string; data?: any; error?: any }[] = [];
